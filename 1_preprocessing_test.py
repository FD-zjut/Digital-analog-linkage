...
DATE_SETS = ["c1","c4","c6"]
HNR = 20
hn_rate = np.power(10,HNR/10)
print(hn_rate)

## 提取28个特征
for now_data_name in DATE_SETS:
    features = []
    for i in range(GENRATE_TIME):
        features.append(np.zeros((315,7,28)))
    path = "D:\gbs\phm_dataset\\"+now_data_name  # 读取csv文件目录路径
    ...
    for fn in FileNames:
        ...
        if re.search(r'\.csv$', fn):
            print(now_data_name,jishu)
            fullfilename = os.path.join(path, fn)
            raw_df = pd.read_csv(fullfilename,encoding='utf-8',on_bad_lines='skip',index_col=False, header=None)
            for g_id in range(GENRATE_TIME):
                df = raw_df.copy()
                for now_sensor in range(df.shape[1]):
                    power = np.mean(np.power(df[now_sensor],2))
                    power2 = power / hn_rate
                    sq_power2 = np.sqrt(power2)
                    noise = np.random.normal(0, sq_power2, df[now_sensor].shape)
                    df[now_sensor] += noise
                # print(raw_df)
                # print(df)
                # print("=")
                ...
                features[g_id][jishu,:,:]=feature
            jishu=jishu+1
    for i in range(len(features)):
        np.save("data\\"+now_data_name + '_features_' + str(i),features[i])
